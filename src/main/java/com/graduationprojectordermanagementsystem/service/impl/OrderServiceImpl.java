...
import com.graduationprojectordermanagementsystem.pojo.dto.PageDTO;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PageResult<OrderVO> getOrderList(Integer pageNum, Integer pageSize) {
        ...
        PageDTO p = createPage(pageNum, pageSize, wrapper);

        log.info("分页查询完成，总记录数：{}，当前页数据量：{}", p.getTotal(), p.getRecords().size());

        // 4. 转换为 VO 列表
        List<OrderVO> voList = p.getRecords().stream().map(order -> {
        ...
        // 5. 构建并返回 PageResult
        return PageResult.of(p.getTotal(), p.getCurrent(), p.getSize(), voList);
    }
    ...
    public PageResult<OrderVO> getOrdersByUserId(Long userId, Integer pageNum, Integer pageSize) {
        ...
        log.info("用户 {} 共有 {} 条订单，当前页返回 {} 条", userId, p.getTotal(), voList.size());
    ...
    public boolean updateOrder(OrderDTO orderDTO) {
        ...
        if (orderDTO == null || orderDTO.getId() == null || orderDTO.getId() <= 0) {
            ...
            throw new BaseException(ResultCode.VALIDATE_FAILED ,"订单ID不能为空且必须大于0");
        }
        ...
        if (existingOrder == null) {
            ...
            throw new BaseException(ResultCode.VALIDATE_FAILED ,"订单不存在");
        }

        // 3. 构建要更新的 Order 对象（只设置非 null 字段）
    ...
    private PageDTO createPage(Integer pageNum, Integer pageSize,LambdaQueryWrapper<Orders>  wrapper){
        // 2. 创建分页对象
    ...
    private static Orders getOrders(OrderDTO orderDTO) {
        Orders order = new Orders();
        order.setId(orderDTO.getId()); // 主键用于 WHERE id=?
        // 只有传了才更新（null 不更新）
        if (orderDTO.getUserId() != null) order.setUserId(orderDTO.getUserId());
        if (orderDTO.getMajorId() != null) order.setMajorId(orderDTO.getMajorId());
        if (orderDTO.getPrice() != null) order.setPrice(orderDTO.getPrice());
        if (orderDTO.getTitle() != null) order.setTitle(orderDTO.getTitle());
        if (orderDTO.getDemand() != null) order.setDemand(orderDTO.getDemand());
        if (orderDTO.getType() != null) order.setType(orderDTO.getType());
